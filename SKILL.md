...
### 0.1 Workspace & Git Repository Scan (Mandatory, Runs Before Branching)

1. **Resolve Target Git Repository Root & Placement Anchor**:
   - Run `git rev-parse --show-toplevel` from the current working directory.
   - If the current directory is not a git repo or is a generic parent directory (e.g. `~/project/`, `~/workspace/`), scan immediate subdirectories for `.git/` (e.g. `ls -d */.git` or `find . -maxdepth 2 -name ".git"`).
   - If a git repository is located in a subfolder (e.g. `Real-Estate-Platform/.git`), that repository root is the **Target Git Repository Root**.
   - **Placement Invariant**: All harness files (`AGENTS.md`, `PROJECT_SPEC.md`, `HARNESS_RATIONALE.md`, `ONBOARDING.md`, `.agents/`) **MUST be written inside the target git repository root**, co-located with `.git/`. They must NEVER be created in a parent workspace directory above the git repository.
2. **List Workspace Root & Manifests**: Examine top-level directories and dependency manifests within the target repository root. Note every directory that looks like a distinct build target or has its own independent manifest (a candidate sub-project root) — item 3 needs this list before it searches.
3. **Search for Prior Harness**: Check if a structurally complete, versioned harness with this builder's fingerprint exists.
   - **Concrete Fingerprint Definition**: A workspace has a prior harness from this builder if, and only if, the host-appropriate `harness_log.json` (per the detected host's primitives file, e.g. `.agents/harness_log.json`, `.claude/harness_log.json`, `.cursor/harness_log.json`, `.windsurf/harness_log.json`) exists inside the target repository root, parses as valid JSON, and contains at least one entry with a `version` field matching the schema in `decision_procedure.md`'s Versioning & Migration Protocol (i.e. an `add`/`rename`/`merge`/`split`/`remove`/`factual-fix`/`behavior-change` changelog, not an arbitrary log file of the same name). This is the single authoritative signal — do not infer a prior harness from the presence of `AGENTS.md`/`CLAUDE.md` alone, since those files can pre-date this builder or be hand-written.
   - **Search Every Candidate Root, Not Just the Repository Root (Monorepo Case)**: If item 2 surfaced more than one candidate sub-project root, run this fingerprint search under *each* candidate root independently, in addition to the repository root — not only at the top level. A monorepo can have a versioned harness scoped to one sub-project (e.g. `packages/api/.claude/harness_log.json`) with nothing at the repository root; searching only the root before item 4 has formally asked about split/unified scope is how that harness goes undetected and the sub-project gets misrouted into Genesis or Adoption instead of Audit. Carry forward a per-root result (found / not found / corrupted) into item 4.
...
4. **Pre-Existing Directives & Framework Rules Discovery**:
   - Scan the codebase for ANY existing agent directives or rule files: `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `.cursor/rules/`, `.windsurfrules`, etc. across all project subdirectories.
   - If existing files are found, classify each:
     - **Framework-generated blocks**: e.g., Next.js 15+ auto-generated `<!-- BEGIN:nextjs-agent-rules -->` in sub-packages (like `zakaria-farid/AGENTS.md`). Preserve these framework notices, but do NOT treat them as the repository's root orchestrator!
     - **Sub-package directives**: Document if they should remain scoped to that sub-package or be reconciled into the root.
     - **Legacy directives**: Plan to replace or merge them into the single authoritative `<repo_root>/AGENTS.md`.
   - **Singularity Rule**: There must be exactly ONE root orchestrator directive (`<repo_root>/AGENTS.md`). Never create duplicate competing root files (such as adding `.agents/rules/harness.md` that duplicates `AGENTS.md`).
5. **Monorepo / Sub-project Check**: Check for multiple distinct build targets or independent manifests — regardless of whether the user's invocation named an explicit mode. An explicit `/harness audit` (or genesis/adopt) on a monorepo still needs to know whether it's scoped to one sub-project, all of them, or needs the split/unified decision made first; skipping this check on the explicit-mode path is how a monorepo silently gets treated as one project. If multiple sub-projects are detected, prompt the user via `ask_question`:
...
- **No Unapproved Writes (Zero Premature File Creation)**: Never write harness configs, scripts, or spec files to disk without the Step 5 explicit approval gate, regardless of apparent urgency. Drafting is in-memory only until approved.
- **Repository Root Placement Anchor**: All harness files (`AGENTS.md`, `.agents/`, `PROJECT_SPEC.md`, `HARNESS_RATIONALE.md`, `ONBOARDING.md`) must be written inside the target git repository root, co-located with `.git/`. They must NEVER be placed in a parent workspace directory above the git repository.
- **Zero Absolute Paths (Strict Script Portability)**: Never hardcode machine-specific absolute paths (`/home/...`, `/Users/...`, `C:\...`) into ANY generated script, hook, or configuration. Every script must resolve repository and sub-project paths dynamically via `git rev-parse --show-toplevel` or `$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)`.
- **Single Orchestrator Authority & No Shadow Directives**: Exactly ONE top-level orchestrator directive (`<repo_root>/AGENTS.md`) governs the repository. Never create duplicate competing directives (such as `.agents/rules/harness.md` mirroring `AGENTS.md`). Framework-generated rules (such as Next.js agent blocks in sub-packages) must be preserved in their native sub-packages without conflicting with the root orchestrator.
- **Path-Aware Enforcement Integrity**: The mechanical enforcement script (`verify_gates.sh`) must either (a) inspect staged files to only run heavy domain/invariant test suites when relevant files are touched while running typechecks globally, or (b) `HARNESS_RATIONALE.md` must explicitly document why the git pre-commit hook runs a global safety net while the track's inner-loop verification command runs a lighter subset.
