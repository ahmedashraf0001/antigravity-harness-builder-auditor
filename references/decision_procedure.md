...
**Dual-Gating Without a Hard Invariant (narrower, opt-in path)**: A track may also be dual-gated even though nothing about it meets the Hard Invariant trigger list above — but only when the human explicitly requests the extra scrutiny for a stated reason, never inferred or applied by default (e.g. *"bulk-editing this pricing-copy track isn't irreversible, but a bad batch is expensive to notice, so I want it checked twice anyway"*). Record this the same way an invariant's provenance is recorded: one line in `HARNESS_RATIONALE.md` naming the track, `dual-gated: non-invariant`, and the human's stated reason. This distinction has real downstream consequences — a non-invariant dual-gated track is exempt from the Circuit Breaker ceiling floor (Circuit Breaker Protocol §4 below) and may rely on CI-only enforcement (Mechanical Enforcement Protocol §1(c)) where a Hard-Invariant track may not — so a track cannot be dual-gated with neither a Hard Invariant classification nor a recorded non-invariant reason; Step 4.1 item 12 checks this explicitly.
...
- **Flag Sprawl Explicitly at the Threshold**: If total tracks exceed roughly 8 or total roles exceed roughly 5 for a single-project harness, this is not an error, but it must be surfaced to the human directly, with the specific tracks/roles listed, so they can confirm the complexity is warranted rather than discovering it later by feel:
...
- **Unified Monorepo Harnesses Get a Scaled Threshold, Not an Exemption**: If Step 0 resolved a "unified harness" scope spanning K sub-projects, the single-project 8/5 guideline does not apply as-is — a unified harness legitimately grows with each sub-project it folds in. Use `8 × K` tracks / `5 × K` roles as the flag point instead, and when reporting at that threshold, break the list down by originating sub-project rather than only as a flat combined list, so the human can judge sprawl per sub-project as well as in aggregate:
  > *"This unified harness spans 3 sub-projects and has grown to 21 tracks (`packages/api`: 9, `packages/web`: 7, `packages/billing`: 5) / 13 roles. Does this match the actual number of distinct risk profiles, or should some of these be consolidated within a sub-project?"*
  A single sub-project that alone exceeds the single-project 8/5 guideline is still called out by name, even if the unified total sits under its own scaled threshold — the scaled number governs the aggregate, not any one sub-project's own sprawl.
...
- **Reasoning Effort / Model Tier**:
  - Assign model tiers strictly to `flash` (or `inherit`). Explicitly forbid legacy `pro` defaults across all subagents, as current `flash` models consistently demonstrate superior responsiveness, sharp instruction adherence, tool reliability, and execution speed without the regressions observed in older Pro tiers.
