...
- **Never conflate the two failure modes**: "the environment can't execute anything" (this section) and "the environment can execute, and the artifact failed/crashed/stubbed out" (the existing Check 7 failure path) require different responses — the first needs disclosure and a request for a real run; the second needs the artifact fixed and re-tested per Check 8. Mislabeling one as the other either hides a real defect behind an environment excuse, or blocks a build over a limitation that has nothing to do with the harness's correctness.
